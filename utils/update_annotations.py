...
class UpdateAnnotations:
    ...
    def update_height_order(self):
        ...
        for chrom in range(1, 25):
            chrom = "X" if chrom == 23 else "Y" if chrom == 24 else str(chrom)
            annotations = self.collection.find(
                {"chrom": chrom, "source": self.file_name}
    ...
    def set_missing_fields(self, annotation):
        ...
        for key in self.fields_to_save:
            if key in annotation.keys():
                ...
                print(
                    "Warning, field {} is missing from annotation {} in file {}".format(
                        key, annotation, self.file_name
                    )
                )
...
def format_field(title, field):
    ...
    if "chrom" in title:
        if not field:
            print(
                '"{}" is not defined. Value was "{}" Skipping line:'.format(
                    title, field
                )
            )
...
def main():
    ...
    parser.add_argument(
        "-f",
        "--file",
        help="A file to parse or a directory of files to parse",
        required=True,
    )
    parser.add_argument("-hg", "--hg_type", help="Set hg-type", default="38")
    parser.add_argument(
        "-c", "--collection", default="annotations", help="Optional collection name"
    )
    ...
    for annot_file in files:
        ...
        if ".bed" in annot_file:
            ...
            print(f"Wrong file type: {annot_file}")
            continue
    print("Finished")
