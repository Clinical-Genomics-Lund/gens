...
import { showTooltip, hideTooltip, hideFeatureInTooltip, createTooltipElement } from './tooltip.js'
    ...
    this.trackContainer.addEventListener('mousemove',
      (event) => {
        ...
        for (const element of this.geneticElements) {
          ...
          // if element is allready displayed
          if (element.tooltip.isDisplayed) {
            // hide if mouse is no longer within element
            if (isInElement) {
              ...
              for (const feature of element.features) {
                ...
                if (isInFeature && !feature.isDisplayed) {
                  // show feature
                  showTooltip({ tooltip: element.tooltip, featureId: feature.id })
            ...
            if (isInElement) {
              ...
              for (const feature of element.features) {
                ...
                if (isWithinElementBbox({ element: visableFeat, point })) {
              ...
              showTooltip({ tooltip: element.tooltip, featureId: selectedFeature })
            }
          }
        }
      })
