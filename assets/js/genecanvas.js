...
export function drawData (scene, data, color) {
  ...
  const material = new PointsMaterial({ size: 2, color: color, transparent: false })
...
export function drawVerticalTicks (scene, canvas, renderX, canvasX, y, xStart, xEnd,
...
// The amplitude scales the values to drawing size
export function drawGraphLines (scene, x, y, yStart, yEnd, stepLength, yMargin, width, height) {
...
export function drawLine (scene, x, y, x2, y2, thickness, color) {
  x = Math.floor(x) + 0.5
  ...
  const material = new LineBasicMaterial({ color: color, linewidth: thickness })
...
function drawBox (scene, x, y, width, height, lineWidth, color, open) {
  ...
  const geometry = new BufferGeometry().setFromPoints(coordAxes)
  const box = new Line(geometry, material)
  scene.add(box)
}
