...
from ..io.annotation import (parse_annotation_entry, parse_annotation_file,
                             update_height_order)
...
def sample(sample_id, genome_build, baf, coverage, overview_json):
    ...
    if len(get_indexes(gens_db, SAMPLES_COLLECTION)) == 0:
        create_index(gens_db, SAMPLES_COLLECTION)
    # load samples
    sample_obj = Sample(
        sample_id=sample_id,
