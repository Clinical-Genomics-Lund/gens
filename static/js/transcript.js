  ...
  drawTracks (region) {
    $.getJSON($SCRIPT_ROOT + '/_gettrackdata', {
      ...
      if (result['max_height_order'] == 0) {
        ...
        const maxYPos = this.tracksYPos(result['max_height_order'] + 1);
      ...
      for (let i = 0; i < result['tracks'].length; i++) {
        ...
        for (let j = 0; j < track['features'].length; j++) {
          ...
          if (scale * diff >= this.arrowWidth) {
            ...
            this.drawArrow(scale * (latestFeaturePos - result['start_pos'] + diff / 2),
              adjustedYPos, direction, this.featureHeight / 2);
          }
          ...
          switch(feature['feature']) {
            case 'exon':
              ...
              this.drawBand(scale * (feature['start'] - result['start_pos']),
                adjustedYPos, scale * (feature['end'] - feature['start']), this.featureHeight);
              break;
            case 'three_prime_utr':
              this.drawBand(scale * (feature['start'] - result['start_pos']),
                adjustedYPos, scale * (feature['end'] - feature['start']), this.featureHeight / 2);
              break;
          }
        }
      }
